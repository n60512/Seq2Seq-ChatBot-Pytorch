...
corpus = os.path.join("pytorch_tut\data", corpus_name)
...
#%%
t = [random.choice(pairs) for _ in range(5)]
t
#%%
t.sort(key=lambda x: len(x[0].split(" ")), reverse=True)
t
#%%
input_batch =[]
for pair in t:
    input_batch.append(pair[0])
    # output_batch.append(pair[1])
indexes_batch = [indexesFromSentence(voc, input_batch) for sentence in input_batch]
#%%
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    if use_teacher_forcing:
        for t in range(max_target_len):
            ...
            # print(decoder_output.size(),target_variable[t].size(),mask[t].size())
